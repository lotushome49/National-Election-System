...
import { ensureSystemAdminAccount } from "./prisma/bootstrapAdmin";

async function bootstrap(): Promise<void> {
  // 1. Connect to database
  await connectDatabase();

  // 1b. Ensure the documented default system admin exists in the live database
  ...
  const startListening = () => {
    server.listen(port, () => {
      ...
      logger.info(
        `📡 API base: http://localhost:${port}/api/${env.API_VERSION}`,
      );
    });
  };
