...
export const regionService = {
  ...
  async create(dto: CreateRegionDto, actorId: string, ip: string) {
    ...
    const region = await regionRepository.create({ ...dto, createdBy: actorId });
  ...
  async update(id: string, dto: UpdateRegionDto, actorId: string, ip: string) {
    const existing = await regionRepository.findById(id);
    if (!existing) throw new NotFoundError('Region');

    if (dto.name && dto.name !== existing.name && await regionRepository.findByName(dto.name)) {
      throw new ConflictError('Region name already exists');
    }

    if (dto.code && dto.code !== existing.code && await regionRepository.findByCode(dto.code)) {
      throw new ConflictError('Region code already exists');
    }

    const updated = await regionRepository.update(id, { ...dto, updatedBy: actorId });
  ...
  async remove(id: string, actorId: string, ip: string) {
    ...
    if (districtCount > 0 || stationCount > 0) {
      throw new BadRequestError('Cannot delete a region that still has districts or polling stations');
    }
