...
export const votingController = {
  issueToken: async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await votingService.issueToken(electionId, voterId, req.user!.sub, req.ip ?? '');
  ...
  castVote: async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const result = await votingService.castVote(req.body, req.user!.sub, req.ip ?? '');
      sendCreated(res, result, 'Vote cast successfully');
    } catch (err) { next(err); }
  },

  verifyReceipt: async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
