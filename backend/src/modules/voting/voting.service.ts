...
export const votingService = {
  ...
  async issueToken(electionId: string, voterId: string, actorId: string, ip: string) {
    const election = await electionRepository.findById(electionId);
    if (!election) throw new NotFoundError('Election');
    if (election.status !== 'VOTING_OPEN') {
      throw new BadRequestError('Voting is not currently open for this election');
    }

    const voter = await voterRepository.findById(voterId);
    if (!voter) throw new NotFoundError('Voter');
    if (!(voter as any).isVerified) throw new ForbiddenError('Voter is not verified');

    // Prevent duplicate token issuance
    const existing = await votingRepository.findToken(electionId, voterId);
    if (existing) throw new ConflictError('A voting token has already been issued to this voter');
    ...
    await votingRepository.createToken({
      id: uuidv4(), electionId, voterId,
      issuedToUser: actorId, tokenHash, expiresAt, ipAddress: ip,
    });

    await auditService.log({
      userId: actorId, action: 'TOKEN_ISSUED', entity: 'VotingToken',
      entityId: voterId, description: `Token issued for election ${electionId}`, ipAddress: ip,
    });
  ...
  async castVote(dto: CastVoteDto, voterId: string, ip: string) {
    ...
    if (token.voterId !== voterId) throw new ForbiddenError('Token does not belong to this voter');
    if (token.status !== 'UNUSED') throw new ConflictError('Voting token has already been used');
    if (token.expiresAt < new Date()) throw new BadRequestError('Voting token has expired');
    ...
    if (existingBallot) throw new ConflictError('Vote already cast with this token');
    ...
    await auditService.log({
      ...
      description: `Vote cast in election ${dto.electionId}`, ipAddress: ip,
    });
  ...
  async verifyReceipt(receiptHash: string) {
    ...
    if (!ballot) throw new NotFoundError('Receipt not found — vote may not have been recorded');
    // Return minimal info — do NOT expose candidateId to preserve anonymity
    return { verified: true, electionId: ballot.electionId, castAt: ballot.castAt };
  },
};
