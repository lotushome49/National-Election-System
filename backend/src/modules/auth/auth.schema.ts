...
function requireFactor<T extends { code?: string; recoveryCode?: string }>(schema: z.ZodType<T>) {
...
export const mfaChallengeSchema = requireFactor(z.object({
...
export const mfaDisableSchema = requireFactor(z.object({
  password: z.string().min(8).max(128),
  ...factorFields,
}));
