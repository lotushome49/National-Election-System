...
export const districtService = {
  ...
  async getById(id: string, requester?: JwtPayload) {
    ...
    assertUserScopeAccess(requester, { regionId: district.regionId, districtId: district.id }, 'districts');
    return district;
  },

  async create(dto: CreateDistrictDto, actorId: string, ip: string, requester?: JwtPayload) {
    ...
    const district = await districtRepository.create({ ...dto, createdBy: actorId });
  ...
  async update(id: string, dto: UpdateDistrictDto, actorId: string, ip: string, requester?: JwtPayload) {
    const existing = await districtRepository.findById(id);
    if (!existing) throw new NotFoundError('District');
    assertUserScopeAccess(requester, { regionId: existing.regionId, districtId: existing.id }, 'districts');

    const targetRegionId = dto.regionId ?? existing.regionId;
    assertUserScopeAccess(requester, { regionId: targetRegionId, districtId: existing.id }, 'districts');

    const region = await regionRepository.findById(targetRegionId);
    if (!region) throw new NotFoundError('Region');

    if (dto.code && dto.code !== existing.code && await districtRepository.findByCode(dto.code)) {
      throw new ConflictError('District code already exists');
    }
    if (dto.name && (dto.name !== existing.name || targetRegionId !== existing.regionId)) {
      const conflict = await districtRepository.findByNameInRegion(dto.name, targetRegionId);
      if (conflict && conflict.id !== existing.id) {
        throw new ConflictError('District name already exists in this region');
      }
    }

    const updated = await districtRepository.update(id, { ...dto, updatedBy: actorId });
  ...
  async remove(id: string, actorId: string, ip: string, requester?: JwtPayload) {
    ...
    if (stationCount > 0) {
      throw new BadRequestError('Cannot delete a district that still has polling stations');
    }
