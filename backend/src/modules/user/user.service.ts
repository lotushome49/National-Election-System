...
export const userService = {
  async list(q: UserQuery) {
    const { data, total } = await userRepository.findAll(q);
  ...
  async create(dto: CreateUserDto, actorId: string, ip: string) {
    ...
    const user = await userRepository.create({ ...dto, passwordHash, createdBy: actorId });
  ...
  async update(id: string, dto: UpdateUserDto, actorId: string, ip: string) {
    ...
    const updated = await userRepository.update(id, { ...dto, updatedBy: actorId });
  ...
  async remove(id: string, actorId: string, ip: string) {
    ...
    await auditService.log({
      ...
      description: 'User soft-deleted', ipAddress: ip,
    });
  },
};
