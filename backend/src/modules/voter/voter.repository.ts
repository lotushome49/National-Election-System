...
export const voterRepository = {
  findAll: async (q: VoterQuery) => {
    ...
    const [data, total] = await Promise.all([
      prisma.voter.findMany({
        where,
        select: {
          id: true, voterId: true, fullName: true, dateOfBirth: true,
          gender: true, phone: true, email: true, regionId: true,
          districtId: true, pollingStationId: true, isVerified: true,
  ...
  findById: (id: string) =>
    prisma.voter.findFirst({
      ...
      select: {
        ...
        regionId: true, districtId: true, pollingStationId: true,
        isVerified: true, registrationDate: true,
      },
    }),
  ...
  findByBiometricHash: (hash: string) =>
    prisma.voter.findFirst({ where: { biometricHash: hash, deletedAt: null } }),
