...
// --- Types ---
type Role = 'VOTER' | 'ADMIN' | 'REGIONAL_ADMIN' | 'DISTRICT_ADMIN' | 'STAFF' | 'OBSERVER' | 'NONE';
...
const checkPerm = (role: Role, perm: keyof typeof HAS_PERMISSION) => {
  return HAS_PERMISSION[perm].includes(role);
};
...
function CandidateDetailModal({ candidate, onClose, t }: { candidate: Candidate, onClose: () => void, t: any }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
...
export default function App() {
  ...
  const [electionPhase, setElectionPhase] = useState<'REGISTRATION' | 'VOTING' | 'CLOSED'>('REGISTRATION');
  ...
  // Initialize Biometrics (FingerprintJS)
  useEffect(() => {
    const setFp = async () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('results-update', (data: VoteResults) => {
      setResults(data);
    });

    newSocket.on('phase-update', (data: { phase: 'REGISTRATION' | 'VOTING' | 'CLOSED' }) => {
      setElectionPhase(data.phase);
    });

    fetch('/api/election/phase')
      .then(res => {
      ...
      .catch(err => {
        console.error("Failed to fetch initial election phase:", err);
      });
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-12 lg:p-16">
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'voters' && checkPerm(role, 'VIEW_VOTERS') && (
              <VoterRegistryView setView={setView} token={token} t={t} i18n={i18n} />
            )}
            {view === 'users' && checkPerm(role, 'MANAGE_USERS') && (
              <UserManagementView setView={setView} token={token} t={t} i18n={i18n} />
            )}
  ...
  const loginAs = async (role: Role) => {
    ...
    try {
      const response = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({
          role,
          biometricHash: role === 'VOTER' ? fpHash : null,
      ...
      if (role === 'VOTER') setView('voter-hub');
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">{t('auth_required')}</p>
                </div>
              </div>
...
function DashboardView({ results, role, setView, t, user, electionPhase, setElectionPhase, token, i18n }: any) {
  ...
  const [selectedRegion, setSelectedRegion] = useState<string | null>(user?.regionId || user?.assignedRegion || null);
  const [selectedDistrict, setSelectedDistrict] = useState<string | null>(user?.districtId || user?.assignedDistrict || null);
  ...
  const getResultsToDisplay = () => {
    const targetRegion = (user?.role === 'REGIONAL_ADMIN' || user?.role === 'DISTRICT_ADMIN')
      ? (user.regionId || user.assignedRegion)
      : selectedRegion;

    const targetDistrict = user?.role === 'DISTRICT_ADMIN'
      ? (user.districtId || user.assignedDistrict)
      : selectedDistrict;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pb-20">
      ...
      <AnimatePresence mode="wait">
        {displayMode === 'regional' && role === 'ADMIN' && (
          <motion.div
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 p-2 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm">
              ...
              {results.regional?.find((r: any) => r.id === selectedRegion)?.districts?.map((d: any) => (
                <button
                  key={d.id}
                  disabled={role === 'DISTRICT_ADMIN' && d.id !== (user.districtId || user.assignedDistrict)}
...
function VoterRegistryView({ setView, token, t, i18n }: any) {
  const lang = i18n.language as 'en' | 'am';
...
function UserManagementView({ setView, token, t, i18n }: any) {
...
function LogItem({ time, event }: any) {
  return (
